...
def casablanca_cast
  ...
  (
    ...
    join
    (select id from movies where title = 'Casablanca') as casa
  on casa.id = castings.movie_id
  )
...
def alien_cast
  ...
  select actor_id from castings
  join
  (select id from movies where title = 'Alien') as alien 
  on alien.id = castings.movie_id
  )
  SQL
end
