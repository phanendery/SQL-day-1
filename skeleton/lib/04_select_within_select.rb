...
def neighbors_of_certain_b_countries
  ...
  execute(<<-SQL)
    ...
    where continent in (select continent from countries
...
def population_constraint
  ...
  where name = 'Canada') and population  < (select population from countries
  where name = 'Poland')
  SQL
end
...
countries where continent =
  (select c.continent from
(select continent, count(name) as pop from countries 
group by continent) as c
join
 (select continent, count(name) as under_pop from countries where population < 25000000
  group by continent) as c_u 
  on c_u.continent = c.continent  
  where c_u.under_pop = c.pop)
  SQL
end
